...
Primary decoding/order seed: `17`; sensitivity seeds: `29`, `47`.
...
The run is a small, synthetic, falsification-oriented probe. All model judgments are provisional model outputs. No result from this protocol may be described as statistically significant, generalizable, human-gold, or confirmatory.
...
The prediction unit is one unique `(case, method, seed)` attempt. The relation table is a paired diagnostic over predictions, not a new collection of independent cases. Seeds `29` and `47` are repeated measurements of the same 36 cases, not extra sample size.

For each seed, source blocks are deterministically permuted by `SHA256(seed | block_id)`, then cases inside each block by `SHA256(seed | block_id | opaque_case_id)`. The exact schedule is persisted before inference. Atomic and holistic call order is balanced by a precomputed hash rule so neither method is always the cold-start call. Evidence order uses the same seed-derived permutation for both methods while retaining an outer map to original span IDs.

No prompt, threshold, failure policy, relationship, or case text may change between seeds. Seed `17` is collected and sealed first. Seeds `29` and `47` run only if seed 17 is structurally valid; continuation is based on integrity checks, never on whether results look favorable. Sensitivity runs cannot fill in, replace, or average away a failed primary attempt.
...
The atomic request additionally receives the frozen provisional atomic claims, exact declared qualifier `(kind, value)` slots, and required support parts. The holistic request does not receive the decomposition or value list; it receives the same proposition/evidence plus the same generic fidelity rubric covering entity, relation, object, time, scope, condition, quantifier, modality, negation, and attribution. This is detail-matched rather than representation-identical: atomicization is the intended factor.
...
Calls per unique case/seed: one claim-form call shared across methods plus one atomic support call shared with Method C.
...
Calls per unique case/seed: one shared claim-form call plus one holistic support call.
...
Calls per unique case/seed: one shared claim-form call plus the **same cached atomic support response** used by Method A. Method C must not re-query the model for a more favorable matrix.
...
For any support request, transport failure, envelope failure, schema failure, incomplete matrix/audit output, or stale-cache mismatch yields `REVIEW` with a method-specific verifier-failure reason. It cannot reuse a previous successful response. Subject to the systemic stopping rules, the remaining pre-scheduled support roles are still attempted so a claim-form failure or one method failure cannot selectively suppress its comparator.
...
- experiment/run/attempt ID, run class, method/prompt role, seed, opaque and outer case IDs;
...
For 36 unique cases at one seed, the planned physical model work is:
...
For every method/seed and pairwise comparison report:
...
`budget_matched=true` requires the same model digest, seed, decoding, context/output caps, case universe, and support-call count. `observed_compute_matched=true` additionally requires equal successful-call counts and a maximum/minimum ratio no greater than `1.10` for total evaluated tokens. If either flag is false, results are reported as unmatched; latency or cache savings cannot be used to claim method superiority.

## 15. Coverage, risk, and exact reporting

For each method, seed, full 36-case universe, 16-unique-case relationship slice, provenance stratum, and family, report exact:
...
A point is unavailable if any compared method has fewer than `k` native ADMITs. Within an ADMIT tie, select by ascending `SHA256(seed | opaque_case_id)` with no method name in the hash; persist selected IDs and intersections. This deterministic subsampling is an artificial tie policy, not a learned ranking, and matched-count results cannot by themselves establish a better risk-coverage frontier.
...
3. **Incremental-mechanism expectation:** at budget- and observed-compute-matched points, Method C has at least one relationship success that Method B lacks, no reverse relationship failure, and no worse admission risk at the same pre-registered coverage. Otherwise this pilot provides no positive evidence that atomic joint auditing beats a detail-matched holistic audit.
...
5. **Order/seed expectation:** the 16 unique relationship-slice decisions are invariant across seeds 17/29/47 at temperature zero. Any change is reported as backend/order sensitivity, not independent supporting evidence.
...
- coverage, successful-call count, token use, or failure abstention is unmatched;
- the conclusion changes across seeds;
...
The current incremental method claim is killed under the tested conditions if a valid detail-matched Method B weakly dominates Method C at every available pre-registered matched point: no higher risk, no lower coverage, no worse relationship count, and no greater observed compute. It is also killed if Method C fails the duplicate control or repeatedly admits registered corruptions. A kill result triggers negative-result analysis inside locked Direction 1; it does not authorize a pivot to a different research question.
...
7. execute and immediately persist seed-17 requests in the frozen schedule; scoring remains unavailable to the inference process;
8. validate and seal the complete seed-17 raw manifest, then evaluate once;
9. if and only if integrity, not performance, permits continuation, repeat the identical schedule for seeds 29 and 47;
...
- two consecutive transport failures occur, or four or more of 36 attempts for one request role fail;
